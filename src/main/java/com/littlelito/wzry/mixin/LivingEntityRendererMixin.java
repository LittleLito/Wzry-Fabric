...
public abstract class LivingEntityRendererMixin<T extends LivingEntity, M extends EntityModel<T>> extends EntityRenderer<T> {
    ...
    @Shadow public static int getOverlay(LivingEntity entity, float whiteOverlayProgress) {
        return 0;
    }
    ...
    public void render(T livingEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        ...
        if (((LivingEntityAccess) livingEntity).getData().health - livingEntity.getHealth() > 0) {
            hurtTime = ((LivingEntityAccess) livingEntity).getData().age;
            a = ((LivingEntityAccess) livingEntity).getData().health - livingEntity.getHealth();
            ((LivingEntityAccess) livingEntity).getData().health = livingEntity.getHealth();

        }
        if ((((LivingEntityAccess) livingEntity).getData().age - hurtTime) < 20 && (((LivingEntityAccess) livingEntity).getData().age - hurtTime) > 0) {
            matrixStack.translate(0D, 0.5010000467300415D, 0D);
            super.renderLabelIfPresent(livingEntity, Text.of(Float.toString(a)), matrixStack, vertexConsumerProvider, i);
        }
